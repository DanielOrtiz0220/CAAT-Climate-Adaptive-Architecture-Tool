...
class ScoringService {
     ...
     */
    static calculateCurrentScore(request) {
        const { scoringWeights } = config_1.config;
        ...
        const materialsScore = request.materials.reduce((sum, material) => sum + materialScores[material], 0) / request.materials.length;
        ...
        const mitigationScore = Math.min(request.mitigationFeatures.reduce((sum, feature) => sum + mitigationFeatureScores[feature], 0), 100);
        ...
        return (elevationScore * scoringWeights.elevation +
     ...
     */
    static generateTimeline(request) {
        const { baselineFloodParameters } = config_1.config;
        const timeline = [];
        // Ensure we have all required simulation years including 2045 and 2055
        const simulationYears = [...baselineFloodParameters.simulationYears].sort((a, b) => a - b);
     ...
     */
    static generateRecommendations(request, projectedBFE) {
        const recommendations = [];
        const score = this.calculateCurrentScore(request);
        if (score < config_1.config.thresholds.criticalScore) {
            recommendations.push('CRITICAL: Immediate action required to improve flood resilience');
        }
        else if (score < config_1.config.thresholds.warningScore) {
